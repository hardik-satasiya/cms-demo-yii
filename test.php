...
function painty($str) //html2img
{
	...
	$i = 0;
	$h = 0;
	$width = 200;//$config1['width'];
	$height = 400;//$config1['height'];
	$rows = explode("\r\n", $str);
	...
	for($i = 0; $i < strlen($str); $i++) {
		...
		if($bold || $strong)
		{
			$font = $config1['font_path'].$config1['font_bold'];
		} else {
			$kill_bold = $prev_bold;
			$kill_strong = $prev_strong;
			$font = $config1['font_path'].$config1['font'];
		}
...
$im = painty('<img src="images/loader.gif">');
...
//$text = 'hi there jfklasklfklf';
//$font = "css/calibrib.ttf";
////imagettftext($im, 20, 0, 50, 21, $grey, $font, $text);
//imagettftext($im, 20, 0, 49, 40, $black, $font, $text);

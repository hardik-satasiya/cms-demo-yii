...
class Users extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('usertype_id, username, userpass, remarks', 'required'),
			array('usertype_id', 'numerical', 'integerOnly'=>true),
			array('username, userpass', 'length', 'max'=>20),
			array('remarks', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, usertype_id, username, userpass, remarks', 'safe', 'on'=>'search'),
		);
	}
